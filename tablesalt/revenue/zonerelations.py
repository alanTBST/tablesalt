...
def main():
    ...
    hovedstad = list(range(1001, 1100))
    perms = permutations(hovedstad, 2)

    with ThreadPool(4) as pool:

        results = pool.imap(call_api, [x for x in perms])
        res_dict = {}
    ...
    with open(r'inputdata\zone_relations.msgpack', 'wb') as f:
